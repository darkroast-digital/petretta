...
class MySQLDump
{
    ...
    private $defaultSettings = array('include-tables'     => array(), 'exclude-tables' => array(), 'compress' => false, 'no-data' => false, /* http://dev.mysql.com/doc/refman/5.1/en/mysqldump.html#option_mysqldump_no-data */
    ...
    private function listValues($tablename)
    {
        ...
        if ($this->settings['lock-tables']) $this->db_handler->exec("LOCK TABLES `$tablename` READ LOCAL");
        if ($this->settings['add-locks']) $this->write("LOCK TABLES `$tablename` WRITE;\n");
        $onlyOnce = true;
        $lineSize = 0;
        ...
        if (!$onlyOnce) $this->write(";\n");
        if ($this->settings['add-locks']) $this->write("UNLOCK TABLES;\n");
        if ($this->settings['single-transaction']) $this->db_handler->exec("COMMIT");
        if ($this->settings['lock-tables']) $this->db_handler->exec("UNLOCK TABLES");
        return;
    }
}
