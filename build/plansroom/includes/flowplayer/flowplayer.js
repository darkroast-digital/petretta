    ...
    flowplayer(function (player, root) {
        ...
        player.play = function (i) {
            ...
            player.load(typeof player.conf.playlist[i] === 'string' ?
                ...
                player.conf.playlist[i].map(function (item) {
                    return $.extend({}, item);
                })
            );
            return player;
    ...
    flowplayer(function (player, root) {
        ...
        player.embedCode = function () {
            ...
            if ($.isPlainObject(conf.embed)) {
                ...
                if (conf.embed.skin) {
                    scriptAttrs['data-skin'] = conf.embed.skin;
                }
            }
...
flowplayer(function (api, root) {
    var $ = jQuery, conf = api.conf, origin = conf.swf.indexOf("flowplayer.org") && conf.e && root.data("origin"), domain = origin ? getHost(origin) : location.hostname, key = conf.key;
    if (location.protocol == "file:")domain = "localhost";
    api.load.ed = 1;
    conf.hostname = domain;
    conf.origin = origin || location.href;
    if (origin)root.addClass("is-embedded");
    if (typeof key == "string")key = key.split(/,\s*/);
    if (key && typeof key_check == "function" && key_check(key, domain)) {
        if (conf.logo) {
            root.append($("<a>", {"class": "fp-logo", href: origin}).append($("<img/>", {src: conf.logo})))
        }
    } else {
        var logo = $("<a/>").attr("href", "http://flowplayer.org").appendTo(root), controls = $(".fp-controls", root);
        api.bind("pause resume finish unload", function (e) {
            if (/pause|resume/.test(e.type) && api.engine != "flash") {
                logo.show().css({position: "absolute", left: 16, bottom: 36, zIndex: 99999, width: 100, height: 20, backgroundImage: "url(" + [".png", "logo", "/", ".net", ".cloudfront", "d32wqyuo10o653", "//"].reverse().join("") + ")"});
                api.load.ed = logo.is(":visible");
                if (!api.load.ed) {
                    api.pause()
                }
            } else {
