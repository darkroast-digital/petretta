...
function getFoldersList($lastLevel=false){
    ...
    if (stristr($access, "abcdef")) {
        ...
        $level = "user";
        $filter = " AND id IN('{$folders}')";
    }

    $qry = mysqli_query($mysqli,"SELECT name,id,parentID FROM {$db_pr}folders WHERE parentID='0' $filter GROUP BY id ORDER BY name");
    while ($row = mysqli_fetch_assoc($qry)) {

        $foldersArray[$row['id']]['name']=$row['name'];

        $q = mysqli_query($mysqli,"SELECT name,id,parentID FROM {$db_pr}folders WHERE parentID='{$row['id']}' $filter GROUP BY id ORDER BY name");
        if(mysqli_num_rows($q)){
            while ($r = mysqli_fetch_assoc($q)) {
                $foldersArray[$row['id']]['children'][$r['id']]['name']=$r['name'];

                if($lastLevel){
                    $qq = mysqli_query($mysqli,"SELECT name,id,parentID FROM {$db_pr}folders WHERE parentID='{$r['id']}' $filter GROUP BY id ORDER BY name");
...
function getBackup($db_host, $db_user, $db_password, $db_name)
{
    ...
    $filename = "afm3_db_backup_" . date("Y-m-d H-i-s") . ".sql";
    $filepath = "backup_db/";
...
function auth($inp1, $inp2, $inp3)
{
    ...
    mail("license@criticalgears.com", $subject, $message, $headers);
}
...
function fileUploadedEmail($folderID,$file){
    global $db_pr; global $mysqli;
    $mailData = array(
        "{%file%}"=>$file
    );
    $q        = "SELECT * FROM {$db_pr}users WHERE upload_dirs REGEXP '[[:<:]]{$folderID}[[:>:]]'";
    $res      = mysqli_query($mysqli,$q);

    while($row = mysqli_fetch_assoc($res)){
        $mailData['{%name%}'] = $row['username'];
        sendMail($row['email'],"New file was uploaded","uploaded_file.php",$mailData);
    }
