...
class CustomUploadHandler extends UploadHandler
{
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null, $content_range = null)
    {
        ...
        if (empty($file->error)) {
            ...
            $query = "SELECT notify_upload,notify_email FROM {$db_pr}settings WHERE id='1'";
            $result = mysqli_query($mysqli,$query) or die("err: " . mysqli_error($mysqli) . $query);
            if ($row = mysqli_fetch_assoc($result)) {
                foreach ($row as $key => $value) {
                    $$key = $value;
                }
                if(isset($notify_upload) && isset($notify_email) && $notify_upload=="1"){
                    $mailData['{%name%}'] = "Administrator";
                    $mailData['{%file%}'] = $file->name;
                    sendMail($notify_email,"New file was uploaded","uploaded_file.php",$mailData);
                }
            }
            mysqli_free_result($result);
			/* if uncommented - will send email notification to all users assigned to folder upon new file upload */
            //fileUploadedEmail($this->options['folderID'],$file->name);
        }
        return $file;
    }
