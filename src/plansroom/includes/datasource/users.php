 ...
 */
$aColumns = array('id', 'username', 'accesslevel', 'email', 'quota', 'filesize', 'extensions', 'upload_dirs', 'last_login');
...
while ($aRow = mysqli_fetch_array($rResult)) {
    $row = array();
    for ($i = 0; $i < count($aColumns); $i++) {
        if ($aColumns[$i] == 'upload_dirs') {
            $dirs  = explode(",", $aRow['upload_dirs']);
            $_dirs = array();
            foreach ($dirs as $dir) {
                $folderData = getFolder($dir);
                if (!empty($folderData['name'])) {
                    if (!empty($folderData['parentID'])) {
                        $sfolderData = getFolder($folderData['parentID']);
                        $_dirs[]     = "{$sfolderData['name']}/<b>{$folderData['name']}</b>";
                    } else {
                        $_dirs[] = "<b>{$folderData['name']}</b>";
                    }
                }
            }
            $row[] = join("<br>", $_dirs);
        } elseif ($aColumns[$i] == 'accesslevel') {
            $row[] = $aRow['accesslevel'] == 'abcdefghijklmnopqrstuvwxyz' ? "admin" : "user";
        } elseif ($aColumns[$i] == 'extensions') {
            $row[] = join(", ",explode(",",$aRow['extensions']));
        } elseif ($aColumns[$i] == 'last_login') {
            $row[] = $aRow['last_login'] == '0000-00-00 00:00:00' ? "&nbsp;" : date("Y-m-d H:i", strtotime($aRow['last_login']));
        } else {
            /* General output */
            $row[] = $aRow[$aColumns[$i]];
        }
